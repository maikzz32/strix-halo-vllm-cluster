"""Opt-in fixed-M4 HC fusion. Imported by the hash-guarded source patch only."""
import ctypes
import hashlib
import os
import re
...
def _operation(x: torch.Tensor, w: torch.Tensor, xn: torch.Tensor,
    ...
    if not (tuple(x.shape) == (4, 320) and tuple(w.shape) == (10240, 320)
        ...
        raise ValueError("Invalid HC fusion tensor descriptor")
...
def mix(module, x, xn, original_gate_mix):
    ...
    if (module._strix_up_mix_selected and tuple(x.shape) == (4, 320)
            and tuple(xn.shape) == (4, 10240) and x.dtype == xn.dtype == w.dtype == torch.bfloat16
            and x.is_cuda and x.device == xn.device == w.device
            and x.is_contiguous() and xn.is_contiguous() and w.is_contiguous()):
