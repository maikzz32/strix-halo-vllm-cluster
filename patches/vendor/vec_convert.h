...
// Vendored from kyuz0/amd-strix-halo-vllm-toolboxes (scripts/patch_aiter_headers.py,
// VEC_CONVERT payload), itself derived from ROCm/aiter csrc/include/ck_tile/vec_convert.h.
// gfx1151-patch: 54_aiter_rdna_enable
//
// RDNA 3/3.5 (gfx11xx) scalar fallbacks for CDNA-only packed conversion ISA
// (v_pk_mul_f32, v_cvt_pk_fp8_f32, v_cvt_pk_bf8_f32).
//
// WARNING (correctness hazard): the FP4 conversion helpers
// (amd_assembly_cvt_scalef32_pk_fp4_*) return ZERO-INITIALIZED tensors on any
// arch other than gfx950. Any AITER FP4 path reached on gfx1151 is therefore
// SILENTLY WRONG. Patch 54 keeps AITER FP4 gated off (is_fp4_avail() stays
// gfx950/gfx1250-only); do not relax that gate without replacing these stubs.
// SPDX-License-Identifier: MIT
