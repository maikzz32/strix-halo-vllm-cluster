#!/usr/bin/env python3
"""Runtime patch: Triton hot-path lane for the kpool sparse-attention indexer.

Complements (does not replace) the torch-native lane from
patches/runtime_glm53_kpool_torch.py: that lane stays the gfx1x baseline and
keeps every cold path (pool compress / cache write / tail ring seed+update /
short-causal fills / topk / pool expand). This patch reroutes only the two
profiled hot paths to fused Triton kernels that run on gfx1151 (no AITER, no
DeepGEMM, no _C topk):

  - decode paged MQA logits: one fused kernel replaces the torch lane's giant
    [B, D, S] bf16 paged gather + 32-iteration per-head matmul chain. Paged
    addressing uses the torch lane's num_states semantics: block_table columns
    are num_states-pool units (288 = block_size 1152 / kpool 4) while a cache
    page holds page_size pools (32), so slot = table[seq, col//num_states] *
    num_states + col%num_states and page = slot//page_size. (The upstream
    Triton reference _cp_gather_indexer_quant_cache_kernel indexes columns by
    pool//page_size, which is only correct when num_states == page_size and
    runs past the table width here — the known OOB; not reproduced.)
  - prefill MQA logits + the prefill paged cache gather: a 2-D-tiled kernel
    (each K tile loaded once per BLOCK_M rows) replaces the per-head loop; a
    one-program-per-pool kernel replaces the advanced-indexing gather.
  - pool expand + tail append: the shipped kpool_compress
    expand_pools_and_append_tail kernel (pure integer gather, no AITER)
    replaces the ~25-op torch chain; bit-exact.

The per-row top-k stays on the torch lane (_glm53_kpool_topk_rows) for
exact-selection parity with the golden reference.

Numerics: torch.matmul(bf16, bf16) materializes a bf16 tensor before
.float(), so both kernels round the fp32 dot accumulator back through bf16 —
validated (bench/kpool_triton_validate.py) bit-exact gather, topk pool ids /
expanded token ids exactly equal to the torch lane, logits max rel diff ~3e-7
(fp32 reassociation only). The profiling / dummy-run memory reservations of
the torch lane are left untouched (the Triton lane allocates strictly less).

Gate: VLLM_GFX1X_KPOOL_TRITON=1 on, anything else off; DEFAULT OFF so the
coordinator can A/B against the pure torch lane. Read lazily per call (Ray
applies worker env after module import). Requires the torch lane (its call
sites are the injection anchors). Idempotent, fail-closed (ast.parse before
write).
...
MARKER = "# kpool triton lane version: v1.0"
...
# Call-site anchors inside the v1.4 torch lane helper block (exact leading
# whitespace; each must occur exactly once in the whole file).
CALL_SITES = (
    (
        "                        k_vals, k_scales = _glm53_kpool_cache_gather(\n",
        "                        k_vals, k_scales = _glm53_kpool_cache_gather_dispatch(\n",
    ),
    (
        "                logits = _glm53_kpool_mqa_logits_torch(\n",
        "                logits = _glm53_kpool_mqa_logits_dispatch(\n",
    ),
    (
        "        logits = _glm53_kpool_paged_logits_torch(\n",
        "        logits = _glm53_kpool_paged_logits_dispatch(\n",
    ),
    (
        "                expanded = _glm53_kpool_expand_tail_torch(\n",
        "                expanded = _glm53_kpool_expand_tail_dispatch(\n",
    ),
    (
        "        out = _glm53_kpool_expand_tail_torch(pool_ids, dec_seq, index_kpool)\n",
        "        out = _glm53_kpool_expand_tail_dispatch(pool_ids, dec_seq, index_kpool)\n",
    ),
)


def find_helper_span(s):
    """Locate an already-injected Triton helper block: from its version marker
    up to the torch lane block or the direct_register_custom_op call,
    ...
    ends = [
        e
        for e in (
            s.find(TORCH_MARKER, start + 1),
            s.find(ANCHOR_REGISTER, start + 1),
        )
        if e > start
    ]
    if not ends:
        return None
    return start, min(ends) + 1


def main():
    s = io.open(P, encoding="utf-8").read()

    call_sites_done = all(new in s for _old, new in CALL_SITES)
    if MARKER in s and call_sites_done:
        print("   v1.0 already applied")
        return 0
    if TORCH_MARKER.strip() not in s:
        print(
            f"   ERROR: torch kpool lane not present in {P}; apply "
            "patches/runtime_glm53_kpool_torch.py first"
        )
        return 42
    ...
    if span is not None:
        # older version present: replace the helper block, keep the call sites
        s = s[: span[0]] + HELPER + s[span[1]:]
    else:
        # Insert BEFORE the torch lane block so that a re-application of the
        # torch lane (which replaces its own marker..register span) cannot
        # delete this block.
        i = s.find(TORCH_MARKER)
        if i < 0:
    ...
    for old, new in CALL_SITES:
        ...
        if s.count(old) != 1:
            print(
                f"   ERROR: expected 1 call-site anchor {old.strip()!r}, "
                f"found {s.count(old)}"
            )
            return 42
        s = s.replace(old, new)
    ...
    print("   v1.0 injected (Triton hot paths for the gfx1x kpool indexer lane)")
    return 0
