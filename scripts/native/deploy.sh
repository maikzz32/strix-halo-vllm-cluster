...
DEST=/home/cluster-user/strix-halo-next
...
for host in "${HOSTS[@]}"; do
  remote="cluster-user@$host"
  stage=$(ssh "${SSH_OPTS[@]}" "$remote" "mkdir -p '$DEST/deployments' '$DEST/results' && mktemp -d '$DEST/deployments/release.XXXXXXXX'")
  [[ "$stage" =~ ^/home/cluster-user/strix-halo-next/deployments/release\.[a-zA-Z0-9]{8}$ ]] || { echo "Invalid release directory" >&2; exit 1; }
  STAGES+=("$stage")
...
for i in "${!HOSTS[@]}"; do
  stage=${STAGES[$i]}
  ssh "${SSH_OPTS[@]}" "cluster-user@${HOSTS[$i]}" "python3 '$stage/deploy_release.py' promote --stage '$stage' --destination '$DEST'"
done
