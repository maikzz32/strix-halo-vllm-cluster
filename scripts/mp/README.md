...
**Bilder (Stand 03.09.2026): nicht nutzbar.** Der Vision-Pfad läuft durch, liefert aber inhaltlich falsche Merkmale (Vollrot → „White“), identisch mit `--mm-encoder-tp-mode data` und `--mm-encoder-attn-backend TRITON_ATTN`; Vorverarbeitung, Checkpoint-Gewichte und Kernel-Numerik sind geprüft und in Ordnung. Verdacht: Vision-Integration des `qwen4_exp`-Modellcodes (Deepstack/Embedding-Merge). Bis zur Klärung `MM_LIMIT` leer lassen (Text-Modus).

TP2 auf zwei Nodes (05.09.2026): `TP=2 MASTER=<rank0-ip> GPU_UTIL=0.88 EXTRA_ARGS='--max-num-seqs 64'` (Mamba-Cache hat bei TP2 nur 258 Blöcke) → 31,2 tok/s greedy, TPOT 28,5 ms, KV 310k Tokens; TP4 gleicher Stack 41,0. `start_mp_exp.sh` kennt dafür `TP`, `MASTER`, `GPU_UTIL`.
