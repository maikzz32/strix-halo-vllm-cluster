...
worker() { ok=0; bad=0; while [ $(date +%s) -lt $END ]; do
  code=$(curl -s -m 120 -o /tmp/soak/r$1 -w "%{http_code}" localhost:8000/v1/chat/completions -H "Content-Type: application/json" \
    -d "{\"model\":\"/home/cluster-user/qwen38_flashnext\",\"messages\":[{\"role\":\"user\",\"content\":\"Schreibe $((RANDOM%5+2)) Sätze über das Thema Nummer $RANDOM.\"}],\"max_tokens\":64,\"temperature\":$([ $((RANDOM%2)) -eq 0 ] && echo 0 || echo 0.7)}")
