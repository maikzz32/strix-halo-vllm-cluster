...
a=argparse.ArgumentParser();a.add_argument('--host',default='192.168.1.18',choices=['192.168.1.'+str(n) for n in range(15,19)]);args=a.parse_args()
...
source=(root/'tests/bench_dense_int4_grid.py').read_bytes()
out=root/('results/dense-grid-'+args.host+'-20260910');out.mkdir(exist_ok=False)
...
rows[0].update(host=args.host,container=container,source_sha256=hashlib.sha256(source).hexdigest(),service_counters_unchanged=True)
