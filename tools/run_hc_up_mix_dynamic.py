...
source=(root/'tests/check_hc_up_mix_dynamic.py').read_bytes()
out=args.output;out.mkdir(parents=True,exist_ok=False)
(out/'before.json').write_text(json.dumps(before))
modules={name:(root/'patches'/('hc_up_mix_dynamic.py' if name=='hc_up_mix_runtime.py' else name)).read_text() for name in ('hc_up_mix_runtime.py','hc_up_mix_source.py')}
