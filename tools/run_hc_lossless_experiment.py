#!/usr/bin/env python3
"""Dry-run by default; bounded isolated lossless HC test in the existing Node18 container."""
import argparse
import hashlib
...
def owned(run_id):
 result=[]
 marker=('STRIX_HC_LOSSLESS_RUN_ID='+run_id).encode()
...
assert hashlib.sha256(pathlib.Path(cfg['library']).read_bytes()).hexdigest()==cfg['library_sha256']
root=pathlib.Path('/tmp/strix-hc-lossless-'+cfg['run_id']);root.mkdir()
for name,source in cfg['files'].items():
 root.joinpath(name).write_text(source)
env=dict(os.environ);env['STRIX_HC_LOSSLESS_RUN_ID']=cfg['run_id'];env['TRITON_CACHE_DIR']=str(root/'triton-cache')
p=subprocess.run(['timeout','--signal=TERM','--kill-after=5s','150s','python3',str(root/'bench_hc_lossless.py'),'--output',str(root/'result.json'),'--library',cfg['library']],env=env,capture_output=True,timeout=160)
...
print('STRIX_HC_LOSSLESS_RESULT='+json.dumps(record),flush=True)
...
def main():
    ...
    paths = [ROOT/'tests'/name for name in ('bench_hc_lossless.py',)]
    files = {p.name:p.read_text(encoding='utf-8') for p in paths}
    cfg = {'run_id':uuid.uuid4().hex, 'files':files, 'prior_run_id':args.prior_run_id,'library':args.library,'library_sha256':args.library_sha256}
    manifest = {'run_id':cfg['run_id'], 'library':args.library, 'library_sha256':args.library_sha256, 'source_sha256':{k:hashlib.sha256(v.encode()).hexdigest() for k,v in files.items()}}
    if not args.execute:
    ...
    rows=[json.loads(line.partition('=')[2]) for line in process.stdout.decode().splitlines()
          if line.startswith('STRIX_HC_LOSSLESS_RESULT=')]
    ...
    for row in (record.get('report') or {}).get('cases',[]):
        print(row['shape'],row.get('reference_bit_mismatches'),row.get('packed_bit_mismatches'),row['methods'])
