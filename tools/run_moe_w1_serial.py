#!/usr/bin/env python3
"""Dry-run by default; bounded isolated W1 serial split-preserving test in the existing Node18 container."""
import argparse
import hashlib
...
def owned(run_id):
 result=[]
 marker=('STRIX_W1_SERIAL_RUN_ID='+run_id).encode()
...
root=pathlib.Path('/tmp/strix-w1-serial-'+cfg['run_id']);root.mkdir()
for name,source in cfg['files'].items():
 root.joinpath(name).write_text(source)
env=dict(os.environ);env['STRIX_W1_SERIAL_RUN_ID']=cfg['run_id'];env['TRITON_CACHE_DIR']=str(root/'triton-cache')
p=subprocess.run(['timeout','--signal=TERM','--kill-after=5s','150s','python3',str(root/'bench_moe_w1_serial.py'),'--output',str(root/'result.json'),'--seed',str(cfg['seed'])],env=env,capture_output=True,timeout=160)
...
print('STRIX_W1_SERIAL_RESULT='+json.dumps(record),flush=True)
...
def main():
    ...
    paths = [ROOT/'tests'/name for name in ('bench_moe_w1_serial.py','moe_w1_serial_kernel.py')]
    ...
    rows=[json.loads(line.partition('=')[2]) for line in process.stdout.decode().splitlines()
          if line.startswith('STRIX_W1_SERIAL_RESULT=')]
