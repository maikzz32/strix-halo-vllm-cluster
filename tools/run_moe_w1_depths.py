#!/usr/bin/env python3
"""Dry-run by default; bounded isolated W1 workgroup-order test in the existing Node18 container."""
import argparse
import hashlib
...
def owned(run_id):
 result=[]
 marker=('STRIX_W1_ORDER_RUN_ID='+run_id).encode()
...
root=pathlib.Path('/tmp/strix-w1-order-'+cfg['run_id']);root.mkdir()
for name,source in cfg['files'].items():
 root.joinpath(name).write_text(source)
env=dict(os.environ);env['STRIX_W1_ORDER_RUN_ID']=cfg['run_id'];env['TRITON_CACHE_DIR']=str(root/'triton-cache')
p=subprocess.run(['timeout','--signal=TERM','--kill-after=5s','150s','python3',str(root/'bench_moe_w1_order.py'),'--output',str(root/'result.json'),'--seed',str(cfg['seed']),'--tokens',str(cfg['tokens'])],env=env,capture_output=True,timeout=160)
...
print('STRIX_W1_ORDER_RESULT='+json.dumps(record),flush=True)
...
def main():
    ...
    parser.add_argument('--seed', type=int, default=9562)
    ...
    paths = [ROOT/'tests'/name for name in ('bench_moe_w1_order.py','moe_w1_order_kernel.py')]
    files = {p.name:p.read_text(encoding='utf-8') for p in paths}
    files['bench_moe_w1_order.py']=(ROOT/'tests/bench_moe_w1_depths.py').read_text()
    ...
    rows=[json.loads(line.partition('=')[2]) for line in process.stdout.decode().splitlines()
          if line.startswith('STRIX_W1_ORDER_RESULT=')]
