...
p=argparse.ArgumentParser();p.add_argument('--output',type=Path,required=True);p.add_argument('--run-id',required=True);p.add_argument('--original',action='store_true');args=p.parse_args()
code=r"""
...
for n,v in r.items():
 assert v['files']=={'qwen_gdn_linear_attn.py':('cb819264c1e791177d866f484ca0579a7d57141926705a1f75dd4a79de4d0125' if args.original else '409969ebb81cc4b5adec548271fff60ecf6ead91adc0b7328f5450685c6646f5'),'_strix_gdn.py':'a064bdbab5d4e0022af40d7c09b86e3a0c4fd258befe31948f5e685f2395d2b2','_strix_gdn_kernel.py':'5c7ed927e90a5ac7014bc894f35bb3ddbdf619eee5cf2839f152e9c266c293dc'}
 assert len(v['workers'])==1
 env=v['workers'][0]['environment']
 assert env['STRIX_PACKED_GDN'] in (('', '0') if args.original else ('1',))
 assert env['STRIX_MOE_W2_SERIAL']=='1'
