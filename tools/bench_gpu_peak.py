"""Reversible all-node GPU high/profile_peak/high experiment; no manual clocks or power limits."""
import json,subprocess
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
OUT=ROOT/'results/gpu-peak-20260910'
...
if ACTION!='read':
 ...
 assert before['level'] in (ORIGINAL['level'],'profile_peak')
 try:
  p.write_text('profile_peak' if ACTION=='set' else ORIGINAL['level'])
 except BaseException:
  p.write_text(ORIGINAL['level'])
  raise
after=snapshot()
if ACTION=='set':assert after['level']=='profile_peak'
...
def main():
    ...
    assert all(row['level']=='high' for row in original.values())
    ...
    try:
        ...
        print('GPU profile_peak verified on all four nodes',flush=True)
        bench('profile_peak')
    finally:
        errors=[]
        for h in attempted:
    ...
    bench('after')
    reports={n:json.loads((OUT/(n+'.json')).read_text(encoding='utf-8')) for n in ('before','profile_peak','after')}
    ...
    print('GPU_PEAK_EXPERIMENT_COMPLETE',flush=True)
