#!/usr/bin/env python3
"""Dry-run by default; bounded isolated W2 epilogue test in the existing Node18 container."""
import argparse
import hashlib
...
def owned(run_id):
 result=[]
 marker=('STRIX_W2_EPILOGUE_RUN_ID='+run_id).encode()
...
root=pathlib.Path('/tmp/strix-w2-epilogue-'+cfg['run_id']);root.mkdir()
for name,source in cfg['files'].items():
 root.joinpath(name).write_text(source)
env=dict(os.environ);env['STRIX_W2_EPILOGUE_RUN_ID']=cfg['run_id'];env['TRITON_CACHE_DIR']=str(root/'triton-cache')
p=subprocess.run(['timeout','--signal=TERM','--kill-after=5s','150s','python3',str(root/'bench_moe_w2_epilogue.py'),'--output',str(root/'result.json')],env=env,capture_output=True,timeout=160)
...
print('STRIX_W2_EPILOGUE_RESULT='+json.dumps(record),flush=True)
...
def main():
    ...
    paths = [ROOT/'tests'/name for name in ('bench_moe_w2_epilogue.py','moe_w2_reduce_sum_kernel.py')]
    ...
    rows=[json.loads(line.partition('=')[2]) for line in process.stdout.decode().splitlines()
          if line.startswith('STRIX_W2_EPILOGUE_RESULT=')]
