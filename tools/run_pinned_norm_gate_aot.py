...
test = (root / 'tests/check_pinned_norm_gate_aot.py').read_text()
...
payload = (
    ...
    f'(p/"pin_norm_gate_geometry.py").write_text({(root / "patches/pin_norm_gate_geometry.py").read_text()!r})\n'
    f'(p/"pin_norm_gate_aot.py").write_text({(root / "patches/pin_norm_gate_aot.py").read_text()!r})\n'
...
result.update(host=args.host, container=container,
              module_sha256={name:hashlib.sha256((root / 'patches' / name).read_text().encode()).hexdigest() for name in ['pin_norm_gate_geometry.py','pin_norm_gate_aot.py']},
