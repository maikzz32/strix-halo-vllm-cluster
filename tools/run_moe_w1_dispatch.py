...
p=subprocess.run(['timeout','--signal=TERM','--kill-after=5s','150s','python3',str(root/'bench_moe_w1_dispatch.py'),'--output',str(root/'result.json'),'--seed',str(cfg['seed'])],env=env,capture_output=True,timeout=160)
...
def main():
    ...
    paths = [ROOT/'tests'/name for name in ('bench_moe_w1_dispatch.py','moe_w1_order_kernel.py')]
