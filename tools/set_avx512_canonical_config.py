"""Back up/promote/restore the W2 plus AVX512 communication configuration on the four hosts."""
...
if c['action']=='backup':
 ...
 x['environment']['STRIX_HIP_RDMA_LIBRARY']='/opt/strix-halo-next/hip-uma-ring4-20260907-r1/libhip_rdma_backend.so';assert x==json.loads(old)
 data=new if c['action']=='promote' else old
 tmp=p.with_name('cluster.avx512-pending.json');tmp.write_bytes(data);tmp.replace(p)
...
def main():
 ...
 assert c['new_sha']=='ce249995443ed3299c40c7dd6abdf06c8bf013593b16b662d9057b2a07b06366'
