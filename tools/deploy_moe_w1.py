"""Stage/activate/restore the bounded W1 candidate; defaults to dry run."""
import argparse, hashlib, json, subprocess, uuid
from pathlib import Path
from patch_moe_w1 import BASE_SHA,patched
ROOT=Path(__file__).resolve().parents[1]
RELATIVE='usr/local/lib64/python3.12/site-packages/vllm/model_executor/layers/fused_moe/fused_moe.py'
ADDON='/opt/strix-halo-next/moe-w1-bn32-20260907-r1'
REMOTE=r"""
...
if cfg['action']=='stage':
 ...
 tmp=target.with_suffix('.w1-pending');tmp.write_bytes(data);tmp.replace(target)
...
def main():
 ...
 dest=ROOT/'results'/('moe-w1-deploy-'+args.action+'-'+uuid.uuid4().hex);dest.mkdir(parents=True)
