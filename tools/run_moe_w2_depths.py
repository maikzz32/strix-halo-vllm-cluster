#!/usr/bin/env python3
"""Dry-run by default; bounded isolated W2 serial split-preserving test in the existing Node18 container."""
import argparse
import hashlib
...
def owned(run_id):
 result=[]
 marker=('STRIX_W2_SERIAL_RUN_ID='+run_id).encode()
...
root=pathlib.Path('/tmp/strix-w2-serial-'+cfg['run_id']);root.mkdir()
for name,source in cfg['files'].items():
 root.joinpath(name).write_text(source)
env=dict(os.environ);env['STRIX_W2_SERIAL_RUN_ID']=cfg['run_id'];env['TRITON_CACHE_DIR']=str(root/'triton-cache')
p=subprocess.run(['timeout','--signal=TERM','--kill-after=5s','150s','python3',str(root/'bench_moe_w2_serial.py'),'--output',str(root/'result.json'),'--seed',str(cfg['seed']),'--tokens',str(cfg['tokens'])],env=env,capture_output=True,timeout=160)
...
print('STRIX_W2_SERIAL_RESULT='+json.dumps(record),flush=True)
...
def main():
    ...
    parser.add_argument('--seed', type=int, default=9560)
    parser.add_argument('--tokens', type=int, choices=(3,5), required=True)
    ...
    paths = [ROOT/'tests'/name for name in ('bench_moe_w2_serial.py','moe_w2_serial_kernel.py')]
    files = {p.name:p.read_text(encoding='utf-8') for p in paths}
    files['bench_moe_w2_serial.py']=(ROOT/'tests/bench_moe_w2_depths.py').read_text()
    cfg = {'run_id':uuid.uuid4().hex, 'files':files, 'prior_run_id':args.prior_run_id, 'seed':args.seed, 'tokens':args.tokens}
    ...
    process=subprocess.run(['ssh','-o','BatchMode=yes','cluster-user@192.168.1.18',
        'podman exec -i qwen029-tp4 python3 -S -'],input=REMOTE.replace('CONFIG',repr(cfg),1).encode(),
    ...
    rows=[json.loads(line.partition('=')[2]) for line in process.stdout.decode().splitlines()
          if line.startswith('STRIX_W2_SERIAL_RESULT=')]
