#!/usr/bin/env python3
"""Dry-run by default; bounded isolated rotating W1 test in the existing Node18 container."""
import argparse
import hashlib
...
def owned(run_id):
 result=[]
 marker=('STRIX_W1_ROTATING_RUN_ID='+run_id).encode()
...
root=pathlib.Path('/tmp/strix-w1-rotating-'+cfg['run_id']);root.mkdir()
for name,source in cfg['files'].items():
 root.joinpath(name).write_text(source)
env=dict(os.environ);env['STRIX_W1_ROTATING_RUN_ID']=cfg['run_id'];env['TRITON_CACHE_DIR']=str(root/'triton-cache')
p=subprocess.run(['timeout','--signal=TERM','--kill-after=5s','150s','python3',str(root/'bench_moe_w1_rotating.py'),'--output',str(root/'result.json')],env=env,capture_output=True,timeout=160)
...
print('STRIX_W1_ROTATING_RESULT='+json.dumps(record),flush=True)
...
def main():
    ...
    paths = [ROOT/'tests'/name for name in ('bench_moe_w1_rotating.py','bench_moe_w1_geometry.py')]
    ...
    rows=[json.loads(line.partition('=')[2]) for line in process.stdout.decode().splitlines()
          if line.startswith('STRIX_W1_ROTATING_RESULT=')]
