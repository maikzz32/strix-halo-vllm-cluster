...
for d in pathlib.Path('/proc').iterdir():
 ...
 try:
  ...
  workers.append({'pid':int(d.name),'library_mapped':cfg['library'],'w1_flag':env.get(b'STRIX_MOE_W1_BN32',b'0').decode()})
...
print(json.dumps({'rank':cfg['rank'],'run_id':cfg['run_id'],'workers':workers,'library_sha256':sha,'startup_log_snapshot_sha256':hashlib.sha256(raw).hexdigest(),'parity_proof':proof}))
"""
def main():
 p=argparse.ArgumentParser();p.add_argument('--run-id',required=True);p.add_argument('--library',required=True);p.add_argument('--sha256',required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args()
 assert re.fullmatch('[0-9a-f]{32}',a.run_id) and re.fullmatch('[0-9a-f]{64}',a.sha256)
 a.output.mkdir(parents=True,exist_ok=False)
 def one(rank):
  node=rank+15;c='ray-head' if rank==0 else 'ray-worker';cfg={'rank':rank,'run_id':a.run_id,'library':a.library,'sha':a.sha256}
