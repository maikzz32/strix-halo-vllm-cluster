...
for d in pathlib.Path('/proc').iterdir():
 ...
 try:
  ...
  workers.append({'pid':int(d.name),'library_mapped':cfg['library'],'w1_flag':env.get(b'STRIX_MOE_W1_BN32',b'0').decode(),'w2_flag':env.get(b'STRIX_MOE_W2_SERIAL',b'0').decode(),'w1_order_flag':env.get(b'STRIX_MOE_W1_EXPERT_ORDER',b'0').decode()})
 except FileNotFoundError:continue
assert len(workers)==1 and workers[0]['w1_flag']=='0'
if cfg['w2_flag'] is not None:assert workers[0]['w2_flag']==cfg['w2_flag']
if cfg['w1_order_flag'] is not None:assert workers[0]['w1_order_flag']==cfg['w1_order_flag']
