...
def main():
 ...
 b=subprocess.run(['ssh','-o','BatchMode=yes','cluster-user@192.168.1.18','podman exec ray-worker cat '+r['root']+'/build/librccl-net-uccl.so'],capture_output=True,check=True,timeout=20).stdout
 ...
 for node in (15,16,17,18):
  ...
  q=subprocess.run(['ssh','-o','BatchMode=yes',f'cluster-user@192.168.1.{node}',f'podman exec -i {container} python3 -S -'],input=script.replace('CONFIG',repr(c),1).encode(),capture_output=True,timeout=30)
