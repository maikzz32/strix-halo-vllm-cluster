...
a=argparse.ArgumentParser();a.add_argument('--host',default='192.168.1.18',choices=['192.168.1.'+str(n) for n in range(15,19)]);a.add_argument('--short-rows',type=int,default=3);a.add_argument('--long-rows',type=int,default=4);args=a.parse_args()
...
out=root/(f'results/decode-batch-invariance-{args.short_rows}-{args.long_rows}-'+args.host+'-20260910');out.mkdir(exist_ok=False)
(out/'before.json').write_text(json.dumps(before))
p=subprocess.run(['ssh','-o','BatchMode=yes','maik@'+args.host,f'podman exec -i {container} timeout --signal=TERM --kill-after=5s 150s python3 - --short-rows {args.short_rows} --long-rows {args.long_rows}'],input=source,capture_output=True,timeout=175)
