...
def analyze(path):
    ...
    actual=[sum(v for k,v in d['metric_deltas'].items() if 'spec_decode_num_accepted_tokens_per_pos_total{' in k and f'position="{i}"' in k) for i in range(3)]
    prefix=[sum(n>=i+2 for n in g[1:-1]) for i in range(3)]
    terminal=[a for a in range(4) if [prefix[i]+int(a>i) for i in range(3)]==actual and a+1>=g[-1]]
    checks={'singleton_first':g[0]==1,'bounded_groups':all(1<=n<=4 for n in g),'groups_match_drafts':len(g)-1==metric('spec_decode_num_drafts_total'),'positions_match_with_terminal_truncation':len(terminal)==1}
    ...
    return {'name':path.stem,'raw_sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'usage':d['usage'],'accounting_checks':checks,'terminal_accepted_count':terminal[0],'groups':g,'conditional_results':rows}

def main():
    a=argparse.ArgumentParser();a.add_argument('inputs',type=Path,nargs='+');a.add_argument('--output',type=Path,required=True);args=a.parse_args()
    report={'cases':[analyze(p) for p in args.inputs],'note':'Descriptive K3 trajectories only. Drop initial and final group from transitions. Required saving = third-token successes / total next-group tokens, under an idealized unchanged-state comparison E2=E3-p3. Switching depth changes future boundaries and trajectories; this is not a measured K2 or adaptive speedup. Aggregate accounting is consistent with iteration grouping, not an independent engine trace.'}
    args.output.write_text(json.dumps(report,indent=2)+'\n')
    for case in report['cases']:print(case['name'],case['accounting_checks'],case['conditional_results'][1])
