...
OUT="/home/cluster-user/strix-halo-next/results/$TAG"
mkdir -p "$OUT"
...
podman exec ray-head vllm bench serve \
  ...
  --base-url http://localhost:8000 --model /home/cluster-user/qwen38_rest \
  --dataset-name sharegpt \
  --dataset-path /home/cluster-user/datasets/ShareGPT_V3_unfiltered_cleaned_split.json \
...
podman cp "ray-head:/tmp/codex-benchmark-$TAG/result.json" "$OUT/result.json"
