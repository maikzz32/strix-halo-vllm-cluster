"""Rotating-expert W1 explicit loop pipelining; unchanged dot/reduction order."""
...
def main():
 ...
 os.environ['STRIX_MOE_W1_EXPERT_ORDER']='1'
 import ast,tempfile
 node=next(n for n in ast.parse(source).body if isinstance(n,ast.FunctionDef) and n.name=='w1_expert_kfast')
 kernel_source=ast.get_source_segment(source,node)
 assert kernel_source.count('for i in range(k_iters):')==1
 kernels={}
 for stages in (3,4):
  body=kernel_source.replace('for i in range(k_iters):',f'for i in _glm53_tl.range(k_iters, num_stages={stages}):')
  path=Path(tempfile.mkdtemp(prefix='w1-pipeline-'))/'kernel.py'
  path.write_text('import triton as _glm53_triton\nimport triton.language as _glm53_tl\n@_glm53_triton.jit\n'+body+'\n')
  spec=importlib.util.spec_from_file_location(f'w1_pipeline_{stages}',path)
  mod=importlib.util.module_from_spec(spec);spec.loader.exec_module(mod)
  kernels[stages]=mod.w1_expert_kfast
 dispatched={3:0,4:0}
 class Staged:
  def __init__(self,stages):self.stages=stages
  def __getitem__(self,grid):
   def launch(*args,**kw):
    kw['num_stages']=self.stages;dispatched[self.stages]+=1
    return kernels[self.stages][grid](*args,**kw)
   return launch
 def staged_call(stages,**kw):
  original=fm.w1_expert_kfast
  try:
   fm.w1_expert_kfast=Staged(stages)
   before=dispatched[stages]
   result=fm._glm53_moe_int4_gemv(**kw)
   assert dispatched[stages]==before+1
   return result
  finally:fm.w1_expert_kfast=original
 variants={'installed':fm._glm53_moe_int4_gemv,'stages3':lambda **kw:staged_call(3,**kw),'stages4':lambda **kw:staged_call(4,**kw)}
