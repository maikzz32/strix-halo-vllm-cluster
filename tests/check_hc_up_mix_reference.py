"""Original-only custom-op control: real HC, dynamic shapes and graph replay."""
import os
...
def make(prefix,combine):
    ...
    assert not any('_strix' in key for key, _ in m.named_buffers())
    return m
report=[]
...
dispatch_checks=[]
for rows in (2,4,3,5,16,128,512,4):
    ...
    out=compiled(x,xn)
    assert torch.equal(out,base.hc_gate_mix(xn,m.input_mix_weight_up(x),4))
    dispatch_checks.append(dict(rows=rows,exact=True))
assert len(captured)==1,captured
assert any('strix_hc_up_mix' in n for n in captured[0]),captured
print('RESULT='+json.dumps(dict(status='passed',dynamic_dispatch=dispatch_checks,dynamic_graph_count=len(captured),real_constructor=True,inductor_pair=True,inductor_full_methods=True,checks=report,compiled_fullgraph=True,changed_graph_exact=True)),flush=True)
