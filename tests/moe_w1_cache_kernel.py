...
@_glm53_triton.jit
def w1_cg(x_ptr, b_ptr, bs_ptr, bz_ptr, part_ptr, sorted_ids_ptr, expert_ids_ptr, ntp_ptr, num_valid, stride_xm, N: _glm53_tl.constexpr, K: _glm53_tl.constexpr, GS: _glm53_tl.constexpr, TOPK: _glm53_tl.constexpr, BM: _glm53_tl.constexpr, BN: _glm53_tl.constexpr, BK: _glm53_tl.constexpr, SPLITK: _glm53_tl.constexpr):
    ...
    for i in range(k_iters):
        ...
        b8 = _glm53_tl.load(b_ptr + expert * (N * (K // 2)) + offs_n[:, None] * (K // 2) + offs_kw[None, :], cache_modifier='.cg')
        ...
        scg = _glm53_tl.load(bs_ptr + expert * (N * (K // GS)) + offs_n[:, None] * (K // GS) + offs_g[None, :], cache_modifier='.cg').to(_glm53_tl.float32)
        zg = _glm53_tl.load(bz_ptr + expert * (N // 2 * (K // GS)) + offs_n[:, None] // 2 * (K // GS) + offs_g[None, :], cache_modifier='.cg')
...
@_glm53_triton.jit
def w1_cv(x_ptr, b_ptr, bs_ptr, bz_ptr, part_ptr, sorted_ids_ptr, expert_ids_ptr, ntp_ptr, num_valid, stride_xm, N: _glm53_tl.constexpr, K: _glm53_tl.constexpr, GS: _glm53_tl.constexpr, TOPK: _glm53_tl.constexpr, BM: _glm53_tl.constexpr, BN: _glm53_tl.constexpr, BK: _glm53_tl.constexpr, SPLITK: _glm53_tl.constexpr):
    """Split-K dequant GEMV partials. Same semantics as
    fused_moe_kernel_gptq_awq's K-loop body ((nibble - zp) * scale in fp32,
    bf16 cast, fp32-acc dot), with coalesced packed loads and the K range
    interleaved across SPLITK programs. K % (BK*SPLITK) == 0 and BK % GS == 0
    ...
    for i in range(k_iters):
        ...
        a = _glm53_tl.load(x_ptr + x_row[:, None] * stride_xm + offs_k[None, :], mask=token_mask[:, None], other=0.0)
        offs_kw = kb // 2 + _glm53_tl.arange(0, BKW)
        b8 = _glm53_tl.load(b_ptr + expert * (N * (K // 2)) + offs_n[:, None] * (K // 2) + offs_kw[None, :], cache_modifier='.cv')
        lo = (b8 & 15).to(_glm53_tl.float32)
        hi = (b8 >> 4).to(_glm53_tl.float32)
        nib = _glm53_tl.interleave(lo, hi)
        offs_g = kb // GS + _glm53_tl.arange(0, BKG)
        scg = _glm53_tl.load(bs_ptr + expert * (N * (K // GS)) + offs_n[:, None] * (K // GS) + offs_g[None, :], cache_modifier='.cv').to(_glm53_tl.float32)
        zg = _glm53_tl.load(bz_ptr + expert * (N // 2 * (K // GS)) + offs_n[:, None] // 2 * (K // GS) + offs_g[None, :], cache_modifier='.cv')
        zpg = (zg >> offs_n[:, None] % 2 * 4 & 15).to(_glm53_tl.float32)
        sc = _glm53_tl.reshape(_glm53_tl.broadcast_to(scg[:, :, None], (BN, BKG, GS)), (BN, BK))
        zp = _glm53_tl.reshape(_glm53_tl.broadcast_to(zpg[:, :, None], (BN, BKG, GS)), (BN, BK))
        b = _glm53_tl.trans(((nib - zp) * sc).to(_glm53_tl.bfloat16))
