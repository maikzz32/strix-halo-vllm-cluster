"""Isolated W1 split-preserving fusion, derived from pinned installed vLLM source.
...
def w1_serial(
    ...
    for split in _glm53_tl.static_range(SPLITK):
        partial = _glm53_tl.zeros((BM, BN), dtype=_glm53_tl.float32)
        for step in range(K // (BK * SPLITK)):
            kb = split * BK + step * BK * SPLITK
            ...
            partial = _glm53_tl.dot(a, b, acc=partial)
