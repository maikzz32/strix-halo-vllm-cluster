...
assert source.count('num_warps = 4') == 1
path.write_text(source.replace(old, 'min(triton.next_power_of_2(V), 128)').replace(
    'num_warps = 4', f'num_warps = {args.warps}'))
...
for seed in (9611, 9612, 9613, 9614):
    ...
    states = [initial.clone() for _ in range(4)]
    gate = torch.randn(48, 128, device="cuda", dtype=torch.bfloat16)
    ...
    def call(i):
        ...
        if i >= 2:
            result = rmsnorm_fn(result.reshape(48, 128), weight, None, z=gate, eps=1e-6, group_size=None, norm_before_gate=True, activation="sigmoid")
        return result
    for count in (1, 2, 3, 4):
        accepted.fill_(count)
        for state in states:
            state.copy_(initial)
        output = [call(i) for i in range(4)]
        torch.cuda.synchronize()
        rows.append(dict(seed=seed, accepted=count,
                         norm_output_differences=int(torch.count_nonzero(output[2] != output[3])),
    ...
    if seed == 9611:
        ...
        for state in states:
            state.copy_(initial)
        expected = [call(i).clone() for i in range(4)]
        ...
        assert all(torch.equal(output[i], expected[i]) and
                   torch.equal(states[i], expected_states[i]) for i in range(4))
        times = [[], [], [], []]
        ...
        for round_id in range(8):
            for i in ((0, 1, 2, 3) if round_id % 2 == 0 else (3, 2, 1, 0)):
...
print('RESULT=' + json.dumps(dict(status='passed', cases=rows,
    ...
    labels=['BV32', 'BV128', 'BV32+norm', 'BV128+norm'],
    original_warps=4, candidate_warps=args.warps,
    note='Sigmoid RMSNorm, separate operators captured together; no fusion.')))
