...
def main():
 p=argparse.ArgumentParser();p.add_argument('--output',type=Path,required=True);p.add_argument('--seed',type=int,default=9562);p.add_argument('--tokens',type=int,choices=(3,5),required=True);args=p.parse_args()
 import torch
 ...
 os.environ['STRIX_MOE_W1_EXPERT_ORDER']='0'
 ...
 from moe_w1_order_kernel import w1_expert_kfast,w1_expert_nfast
 def cache_call(kernel,**kw):
  original=fm._glm53_moe_int4_gemv_partial
  try:
   fm._glm53_moe_int4_gemv_partial=kernel
   return fm._glm53_moe_int4_gemv(**kw)
  finally:fm._glm53_moe_int4_gemv_partial=original
 variants={'installed':fm._glm53_moe_int4_gemv,'expert_kfast':lambda **kw:cache_call(w1_expert_kfast,**kw)}

 E,N,K,GS,M,TOPK,BM=512,320,2560,32,args.tokens,10,16
