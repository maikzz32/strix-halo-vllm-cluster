...
def main():
    ...
    assert hashlib.sha256(a.library.read_bytes()).hexdigest()=='ae7c17353a341bca79219f4a22346ba9a96ee5a4202932d8bc6609cc675b0344'
    import torch
    from vllm.model_executor.layers.utils import rocm_unquantized_gemm_impl
    torch.set_num_threads(1);torch.manual_seed(9501)
    ...
    report={'status':'running','cases':[],'note':'Synthetic BF16 weights, 24-matrix pools; grid parameter is launch geometry, not a changed physical CU count.'}
    for n,k in ((336,10240),(320,10240),(10240,320)):
        ...
        methods=[0,10,20,30,40,60,80]
        def call(grid,w):
            if grid==0:return rocm_unquantized_gemm_impl(x,w)
            y=torch.empty((4,n),device='cuda',dtype=torch.bfloat16)
            rc=lib.hc_run(0,w.data_ptr(),x.data_ptr(),y.data_ptr(),n,k,grid,torch.cuda.current_stream().cuda_stream,None,None)
            assert rc==0
            return y
