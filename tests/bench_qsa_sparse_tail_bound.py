"""Bound the original sparse GQA loop using the last nonnegative index."""
import hashlib
...
'''
    candidate_source = source.replace(marker, bound + marker)
    with tempfile.TemporaryDirectory(prefix='qsa-sparse-empty-') as folder:
        ...
        packed_cache = torch.randn((64, 64, 1, 512), device='cuda', dtype=torch.bfloat16)
        cache_k, cache_v = packed_cache.split(256, dim=-1)
        table = torch.stack([torch.randperm(64, device='cuda', dtype=torch.int32) for _ in range(2)])
        for rows in (4, 8, 504, 1024):
            ...
            for call in calls:
                ...
                with torch.cuda.graph(graph):
                    call()
                graphs.append(graph)
            for pattern in ('empty', 'causal', 'holes', 'full', 'invalid_page', 'expanded_short', 'expanded_mixed', 'expanded_long'):
                ...
                if pattern == 'empty':
                    ...
                    req[0] = -1
                elif pattern.startswith('expanded_'):
                    req.copy_(torch.arange(rows, device='cuda', dtype=torch.int32) % 2)
                    positions = torch.arange(rows, device='cuda', dtype=torch.int64)
                    if pattern == 'expanded_mixed':
                        positions += req * 2048
                    elif pattern == 'expanded_long':
                        positions += 2045
                    lengths = torch.full((2,), 4096, device='cuda', dtype=torch.int32)
                    # Use installed expansion, with permuted complete block ranks.
                    # The short-prefix count limits accesses to selected blocks.
                    blocks = torch.full((rows, 512), -1, device='cuda', dtype=torch.int32)
                    for row, position in enumerate(positions.cpu().tolist()):
                        complete = (position + 1) // 4
                        count = min(complete, 512)
                        if count:
                            blocks[row, :count] = torch.randperm(complete, device='cuda', dtype=torch.int32)[:count]
                    original.expand_qsa_block_indices_cuda(blocks, positions, lengths, req, 4, 2048, indices)
