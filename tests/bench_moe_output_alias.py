"""Compare actual WNA16 modular apply with/without its final output copy."""
import argparse
import ast
import json
import os
...
def main():
    ...
    from moe_output_alias import transform
    torch.set_num_threads(1)
    torch.manual_seed(9401)
    from vllm.v1.worker.workspace import init_workspace_manager
    init_workspace_manager(torch.device('cuda:0'))
    source = transform(Path(mk.__file__).read_text())
    cls = next(n for n in ast.parse(source).body if isinstance(n, ast.ClassDef) and n.name == 'FusedMoEKernelModularImpl')
    method = next(n for n in cls.body if isinstance(n, ast.FunctionDef) and n.name == '_fused_experts')
    namespace = dict(mk.__dict__)
    exec(compile(textwrap.dedent(ast.get_source_segment(source, method)), '<alias-method>', 'exec'), namespace)
    mk.FusedMoEKernelModularImpl._fused_experts = namespace['_fused_experts']
    ...
    for m in (1,4,8,16):
        ...
        def call(mode):
            os.environ['STRIX_MOE_OUTPUT_ALIAS'] = str(mode)
        ...
        with torch.cuda.stream(stream):
            ...
            for rep in range(8):
                for mode in ((0,1) if rep%2==0 else (1,0)):
                    ...
                    start.record()
                    for _ in range(100):graphs[mode].replay()
