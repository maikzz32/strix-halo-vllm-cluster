...
torch.set_num_threads(1); torch.manual_seed(9510)
...
for m in (1,4):
 for n,k in ((512,2560),(320,2560),(2560,160)):
  x=torch.randn((m,k),device='cuda',dtype=torch.bfloat16)
  pool=[(torch.randint(-128,128,(n,k//2),device='cuda',dtype=torch.int8),torch.rand((n,k//32),device='cuda',dtype=torch.bfloat16)*.02,torch.randint(0,16,(n,k//32),device='cuda').to(torch.bfloat16)) for _ in range(24)]
  ...
  with torch.cuda.stream(stream):
   ...
   for r in range(6):
    for grid in (grids if r%2==0 else grids[::-1]):
     ...
     b.record();b.synchronize();samples[grid].append(a.elapsed_time(b)*1000/480)
