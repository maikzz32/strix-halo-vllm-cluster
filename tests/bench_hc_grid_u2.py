...
def main():
    ...
    torch.set_num_threads(1);torch.manual_seed(9502)
    ...
    report={'status':'running','cases':[],'note':'Synthetic BF16 weights, 24-matrix pools; Codes below100 use original U4; codes100+ use U2 with grid=code-100. Grid is launch geometry, not physical CU count.'}
    for n,k in ((336,10240),(320,10240),(10240,320)):
        ...
        methods=[0,20,30,40,120,130,140,160]
        def call(grid,w):
            if grid==0:return rocm_unquantized_gemm_impl(x,w)
            y=torch.empty((4,n),device='cuda',dtype=torch.bfloat16)
            rc=lib.hc_run(4 if grid>=100 else 0,w.data_ptr(),x.data_ptr(),y.data_ptr(),n,k,grid%100,torch.cuda.current_stream().cuda_stream,w.data_ptr(),w.data_ptr())
