...
def main():
    ...
    original_config = tm.try_get_optimal_moe_config
    selected = 0
    def choose(*a, **kw):
        result = dict(original_config(*a, **kw))
        if selected:
            result['BLOCK_SIZE_M'] = selected
            result['BLOCK_SIZE_N'] = 32
            result['BLOCK_SIZE_K'] = 32
        return result
    tm.try_get_optimal_moe_config = choose
    ...
    for m in (64,128,256,512,1024):
        ...
        with torch.cuda.stream(stream):
            ...
            samples={0:[],16:[],32:[]}
            for rep in range(8):
                for mode in ((0,16,32) if rep%2==0 else (32,16,0)):
